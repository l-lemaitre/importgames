<?php
    // Inclut le fichier placé en argument bdd_connection.php à la page page-introuvable.php (ligne 23)
    if(empty($bdd)) {
        $bdd = "bdd_connection";
    ...
    // On récupère le nom du fichier courant pour inclure le contenu de la page page-introuvable.phtml dans le fichier layout.phtml ligne 121
    $_SESSION["template"] = substr($_SESSION["template"], 0, strrpos($_SESSION["template"], "."));


    // Inclut et exécute le fichier layout.phtml qui hérite de la portée des variables présentes dans page-introuvable.php
    if(empty($layout)) {
        $layout = "layout";
    ...
    if(preg_match("/backoff/", $layout)) {
        ...
        // On vérifie que la page est bien sur le "serveur" en utlisant la constante SITE_DIR du fichier dir.php inclut dans le fichier bdd_connection.php (fichiers dir.php ligne 3 et bdd_connection.php ligne 43)
